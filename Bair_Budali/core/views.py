...
def profile(request, pk):
    ...
    if FollowersCount.objects.filter(follower=follower, user=user).first():
        button_text = 'Отследвай'
    else:
        button_text = 'Последвай'
...
def follow(request):
    ...
    if request.method == "POST":
        ...
        if FollowersCount.objects.filter(follower=follower, user=user).first():
            ...
            new_followe = FollowersCount.objects.create(
                follower=follower, user=user)
            new_followe.save()
            return redirect('/profile/' + user)
    else:
...
@login_required(login_url='signin')
def like_post_prof(request):
    #prf_usr = request.POST['user']
    post_id = request.GET.get('post_id')

    post = Post.objects.get(id=post_id)

    like_filter = LikePost.objects.filter(
        post_id=post_id, username=post.user).first()
    if like_filter == None:
        new_like = LikePost.objects.create(post_id=post_id, username=post.user)
        new_like.save()
        post.no_of_likes = post.no_of_likes + 1
        post.save()
